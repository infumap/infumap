...
echo "Forwarded endpoints: /image-extract /image-extract-caption-only /pdf-extract-caption-only /pdf-extract"
echo "Gateway async endpoints: /pdf-extract/jobs"
echo "Global GPU lock: enabled for forwarded endpoint requests"
echo "Global GPU lock lease: ${GPU_GATEWAY_LOCK_LEASE_SECS:-3600}s"
echo "Gateway upstream timeouts: default read/write 30m, /pdf-extract read/write 4h"
echo "Gateway async PDF job upstream timeout: ${GPU_GATEWAY_PDF_EXTRACT_JOB_UPSTREAM_TIMEOUT_SECS:-86400}s"
