...
- "ocr_text" should be an array of at most 5 short readable snippets that identify the subject or context of the image. Do not transcribe documents, tables, or lists — if the image is primarily a document meant to be read in full, use an empty array or just the title/heading. Each snippet must be under 60 characters. Stop after 5 items.
""".strip()
...
def mlx_server_url() -> str:
    ...
    host = env_str("IMAGE_TAGGING_MLX_HOST", "127.0.0.1")
    port = env_int("IMAGE_TAGGING_MLX_PORT", 18080)
    return f"http://{host}:{port}"


def mlx_model_id() -> str:
    return env_str("IMAGE_TAGGING_MODEL_ID", "")


def mlx_model_name() -> str:
    configured = env_str("IMAGE_TAGGING_MLX_MODEL_NAME", "")
    if configured:
        return configured
    model_id = mlx_model_id()
    return Path(model_id).name or model_id or "local-model"
...
def build_runtime_summary() -> list[str]:
    ...
    return [
        ...
        f"mlx_vlm={package_version('mlx-vlm')}",
...
def load_image_embedding_backend() -> tuple[Any, Any, Any, str]:
    try:
        ...
        raise RuntimeError(
            "Image embedding dependencies are unavailable. Install torch, torchvision, and transformers in the image-tagging-mlx venv."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "Image embedding dependencies are incomplete. Install torch, torchvision, and transformers in the image-tagging-mlx venv."
        ) from exc
...
async def lifespan(_: FastAPI):
    ...
    timeout = httpx.Timeout(
        connect=max(5.0, env_float("IMAGE_TAGGING_MLX_CONNECT_TIMEOUT_SECS", 10.0)),
        read=max(30.0, env_float("IMAGE_TAGGING_MLX_READ_TIMEOUT_SECS", 30.0 * 60.0)),
        write=max(30.0, env_float("IMAGE_TAGGING_MLX_WRITE_TIMEOUT_SECS", 60.0)),
    ...
    LOGGER.info("Image tagging (mlx) startup: %s", " ".join(build_runtime_summary()))

    try:
        yield
    finally:
        client = APP_STATE.pop("mlx_client", None)
...
app = FastAPI(
    title="Infumap Image Tagging Service (MLX)",
    version="0.1.0",
    lifespan=lifespan,
    root_path=root_path(),
)
...
def extract_message_text(payload: dict[str, Any]) -> str:
    ...
    if not isinstance(choices, list) or not choices:
        raise ValueError("mlx-vlm response did not contain any choices.")

    choice0 = choices[0]
    if not isinstance(choice0, dict):
        raise ValueError("mlx-vlm response choice was not an object.")
    ...
    raise ValueError("mlx-vlm response message content was empty.")
...
def make_data_url(mime_type: str, file_bytes: bytes) -> str:
...
def build_openai_payload(data_url: str) -> dict[str, Any]:
    return {
        "model": APP_STATE.get("model_name") or mlx_model_name(),
        "messages": [
...
async def post_chat_completion(payload: dict[str, Any]) -> dict[str, Any]:
    client = APP_STATE.get("mlx_client")
    if client is None:
        raise MLXServerError(None, "Image tagging service is not ready.")

    try:
        response = await client.post("/v1/chat/completions", json=payload)
    except httpx.HTTPError as exc:
        raise MLXServerError(None, str(exc)) from exc
    ...
    if response.is_success:
        try:
            ...
            raise MLXServerError(response.status_code, f"Could not parse mlx-vlm JSON response: {exc}") from exc

    raise MLXServerError(response.status_code, body)


async def request_analysis(prepared_bytes: bytes, prepared_mime_type: str) -> dict[str, Any]:
    data_url = make_data_url(prepared_mime_type, prepared_bytes)
    payload = await post_chat_completion(build_openai_payload(data_url))

    try:
        message_text = extract_message_text(payload)
        return extract_first_json_object(message_text)
    except ValueError as exc:
        payload_excerpt = truncate_for_log(json.dumps(payload, ensure_ascii=False))
        LOGGER.error(
            "Could not parse mlx-vlm structured response: error=%s payload=%s",
            exc,
            payload_excerpt,
        )
        raise
...
async def probe_mlx_server() -> tuple[bool, str | None]:
    client = APP_STATE.get("mlx_client")
    if client is None:
        return False, "mlx-vlm client not initialized"
    ...
    return False, "mlx-vlm did not return a successful health response"
...
async def healthz() -> dict[str, Any]:
    ok, detail = await probe_mlx_server()
    return {
        "ok": ok,
        "backend": MLX_BACKEND_NAME,
...
async def tag_upload(request: Request) -> ImageTagResponse:
    ...
    try:
        ...
        async with semaphore:
            ...
            LOGGER.info(
                "Dispatching mlx-vlm run: file=%s size_bytes=%d request_age_ms=%d semaphore_wait_ms=%d",
                file_name,
                upload_size_bytes,
                request_age_ms,
                semaphore_wait_ms,
            )
            parsed, image_embedding = await asyncio.gather(
                request_analysis(prepared_bytes, prepared_mime_type),
        ...
        LOGGER.info(
            "Completed image tagging: file=%s upload_bytes=%d prepared_bytes=%d duration_ms=%d width=%d height=%d prepared_width=%d prepared_height=%d tags=%d embedding_dims=%d backend=%s",
            file_name,
            upload_size_bytes,
            prepared_size_bytes,
            duration_ms,
        ...
        raise
    except (MLXServerError, ValueError) as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    except Exception as exc:
        LOGGER.exception("Image tagging failed: file=%s backend=%s", file_name, MLX_BACKEND_NAME)
        raise HTTPException(status_code=500, detail=str(exc)) from exc
