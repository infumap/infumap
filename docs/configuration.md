...
The GPU tool endpoint names shown in the settings template examples are `/image-extract`, `/text-embed`, and `/pdf-extract`. These optional service URLs are unset by default. The local GPU gateway also keeps `/tag`, `/embed`, and `/convert` as legacy aliases.
