...
pub async fn get_and_validate_session(
  ...
  let session = match db.session.get_session(&session_id) {
    Ok(session_maybe) => match session_maybe {
      ...
      None => {
        debug!(
          "Session '{}' is not available on the server. This can happen if the server is restarted or the session has expired.",
          session_id
        );
        return None;
      }
  ...
  if let Some(session_header) = session_header_maybe {
    if session_header.user_id != session.user_id {
      warn!(
        "Error validating session '{}': Session is for user '{}' not user '{}'.",
        session_header.session_id, session.user_id, session_header.user_id
      );
      return None;
    }
    ...
    let user = match db
      .user
      .get_by_username_case_insensitive(&session_header.username)
      .ok_or(format!("No user exists with username '{}'.", session_header.username))
    {
      Ok(user) => user,
      Err(e) => {
    ...
    if user.id != session_header.user_id {
      warn!(
        "Error validating session '{}': Id for user '{}' is '{}' not '{}'.",
        session_header.session_id, session_header.username, user.id, session_header.user_id
      );
      return None;
    }
  }
