...
use super::routes::command::{serve_chat_stream_route, serve_chat_tool_approval_route, serve_command_route};
...
pub async fn http_serve(
  ...
  let req_origin = req.headers().get(hyper::header::ORIGIN).and_then(|v| v.to_str().ok()).map(|v| v.to_owned());
...
async fn maybe_rotate_primary_session(
  ...
  if let Some(session_header) = incoming_header_session_maybe {
    let rotated_header = InfuSession { username: rotated.username, user_id: rotated.user_id, session_id: rotated.id };
    match serde_json::to_string(&rotated_header).ok().and_then(|header_str| HeaderValue::from_str(&header_str).ok()) {
      Some(header_value) => {
        response.headers_mut().insert(SESSION_HEADER_NAME, header_value);
      }
      None => {
        warn!("Could not set rotated session header for user '{}' session '{}'.", session_header.user_id, session_id);
      }
    }
  }
}
...
pub fn json_response<T>(v: &T) -> Response<BoxBody<Bytes, hyper::Error>>
where
  T: Serialize,
{
...
pub async fn incoming_json_with_limit<T>(request: Request<hyper::body::Incoming>, max_bytes: usize) -> InfuResult<T>
where
  T: DeserializeOwned,
{
  let content_length_maybe = request
    .headers()
  ...
  let body = String::from_utf8(body_bytes).map_err(|e| format!("Request body is not valid UTF-8: {}", e))?;

  Ok(serde_json::from_str::<T>(&body)?)
}

pub async fn incoming_json<T>(request: Request<hyper::body::Incoming>) -> InfuResult<T>
where
  T: DeserializeOwned,
{
