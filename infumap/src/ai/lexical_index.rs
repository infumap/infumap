...
mod tests {
  ...
  use super::{
    FragmentLexicalIndexRebuildMetadata, LexicalFragment, PDF_FRAGMENT_LEXICAL_INDEX_DIR_NAME,
    PDF_FRAGMENT_LEXICAL_INDEX_TEMP_DIR_NAME, TantivyPdfFragmentIndex, pdf_fragment_lexical_index_dir,
    pdf_fragment_lexical_index_temp_dir,
  };

  #[test]
  fn builds_pdf_lexical_index_paths() {
    assert_eq!(
      pdf_fragment_lexical_index_dir("/data/infumap", "abc123").unwrap().to_string_lossy(),
      format!("/data/infumap/user_abc123/indexes/{}", PDF_FRAGMENT_LEXICAL_INDEX_DIR_NAME)
    );
    assert_eq!(
      pdf_fragment_lexical_index_temp_dir("/data/infumap", "abc123").unwrap().to_string_lossy(),
      format!("/data/infumap/user_abc123/indexes/{}", PDF_FRAGMENT_LEXICAL_INDEX_TEMP_DIR_NAME)
    );
  }

  #[tokio::test]
  async fn rebuilds_searches_and_deletes_pdf_fragments() {
    let base_dir = unique_test_dir("rebuild-search-delete");
    let index_dir = base_dir.join("pdf_fragments_tantivy");
    let temp_dir = base_dir.join("pdf_fragments_tantivy.tmp");
    let index = TantivyPdfFragmentIndex::new(index_dir.clone());
    let metadata =
      FragmentLexicalIndexRebuildMetadata { source_digest: "digest-a".to_owned(), expected_fragment_count: 2 };
    let fragments = vec![
      lexical_fragment("item-a", 0, "Pages 1-2\n\nWoodstock was mentioned here.", Some(1), Some(2)),
      lexical_fragment("item-b", 0, "A different passage about other things.", None, None),
    ];

    let status = index.rebuild_from_fragments(&temp_dir, &metadata, &fragments).await.unwrap();
    assert!(status.complete);
    assert_eq!(status.indexed_fragment_count, 2);

    let hits = index.search("Woodstock", 5).await.unwrap();
    assert_eq!(hits.len(), 1);
    assert_eq!(hits[0].item_id, "item-a");
    assert_eq!(hits[0].page_start, Some(1));

    assert_eq!(index.delete_item_fragments("item-a").await.unwrap(), 1);
    assert!(index.search("Woodstock", 5).await.unwrap().is_empty());
  ...
  fn lexical_fragment(
    ...
    LexicalFragment {
      ...
      ordinal,
      source_kind: PDF_MARKDOWN_SOURCE_KIND.to_owned(),
      text: text.to_owned(),
      page_start,
      page_end,
    }
  ...
  fn unique_test_dir(prefix: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("infumap-pdf-lexical-{}-{}-{}", prefix, std::process::id(), nanos))
  }
}
