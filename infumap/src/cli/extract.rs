...
pub async fn execute(sub_matches: &ArgMatches) -> InfuResult<()> {
  ...
  if let Some(container_id) = sub_matches.get_one::<String>("container_id") {
    ...
    let collected_item_ids = collect_extractable_pdf_ids_in_container(db.clone(), container_id).await?;
    let total_candidate_items = collected_item_ids.len();
    let (item_ids, skipped_existing) = if overwrite {
      (collected_item_ids, 0)
    } else {
      filter_extractable_pdf_ids_to_process(&data_dir, db.clone(), collected_item_ids).await?
    };
...
async fn filter_extractable_pdf_ids_to_process(
  data_dir: &str,
  db: Arc<Mutex<Db>>,
  item_ids: Vec<String>,
  ...
  for item_id in item_ids {
    if item_needs_text_extraction(data_dir, db.clone(), &item_id).await? {
...
async fn process_container_pdf_batch(
  ...
  if item_ids.is_empty() {
    if total_candidate_items == 0 {
      ...
      info!(
        "All {} PDFs under container '{}' already have extraction artifacts. Nothing to extract. Use --overwrite to reprocess them.",
  ...
  info!(
    "Starting container-scoped text extraction for container '{}' using '{}' with concurrency {} and delay {:.3}s. Scheduled PDFs: {} (existing skipped: {}, total discovered: {}).",
    container_id,
    text_extraction_url,
    text_extraction_concurrency,
    text_extraction_delay.as_secs_f64(),
  ...
  for worker_index in 0..text_extraction_concurrency {
    ...
    join_set.spawn(async move {
      loop {
        ...
        info!(
          "Container-scoped text extraction worker {} starting PDF '{}'. Pending queue: {}. Progress: {}",
          worker_index + 1,
          item_id,
          queue_remaining,
          progress_summary
  ...
  info!(
    "Container-scoped text extraction finished for container '{}': scheduled={} skipped_existing={} succeeded={} failed={}.",
