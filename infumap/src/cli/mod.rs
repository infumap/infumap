...
impl NamedInfuSession {
  ...
  async fn read_sessions() -> InfuResult<Vec<NamedInfuSession>> {
    ...
    let mut f = File::open(&session_file_path)
      .await
  ...
  async fn write_sessions(sessions: &Vec<&NamedInfuSession>) -> InfuResult<()> {
    let session_file_path = Self::get_cli_sessions_path().await?;
    let mut file = OpenOptions::new().create(true).truncate(true).write(true).open(session_file_path).await?;
  ...
  async fn get_cli_sessions_path() -> InfuResult<PathBuf> {
    let dot_infumap_path =
      PathBuf::from(expand_tilde("~/.infumap").ok_or(format!("Could not determine ~/.infumap path."))?);
    if !path_exists(&dot_infumap_path).await {
      debug!("Creating ~/.infumap directory.");
      tokio::fs::create_dir(&dot_infumap_path)
        .await
    ...
    if !path_exists(&cli_dir_path).await {
      ...
      tokio::fs::create_dir(&cli_dir_path).await.map_err(|e| format!("Could not create cli directory: {}", e))?;
    }
...
fn logout_url_from_base_url(base_url: &str) -> InfuResult<Url> {
  let base_url = Url::parse(base_url).map_err(|e| format!("Could not parse URL: {}", e))?;
