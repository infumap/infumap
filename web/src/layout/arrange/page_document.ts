...
function calcDocumentChildGeometry(
  ...
  if (isComposite(displayItem)) {
    ...
    const extraHeightPx = queryChatCompositeActivityLayoutPx(store, page, displayItem.id, compositeIsCollapsed);
    return CompositeFns.calcGeometry_InDocument(asCompositeItem(displayItem), blockSizePx, displayWidthBl, leftMarginBl, topPx, compositeIsCollapsed, extraHeightPx);
  }
