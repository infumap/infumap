...
import { fullArrange, rearrangeVisualElement, rearrangeWithDisplayId } from "../layout/arrange";
...
function mouseAction_resizing(deltaPx: Vector, store: StoreContextModel) {
  ...
  if (requireArrange) {
    rearrangeVisualElement(store, VeFns.addVeidToPath(VeFns.veidFromVe(activeVisualElement), activeVisualElement.parentPath!));
  }
}
