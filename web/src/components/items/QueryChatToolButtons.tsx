...
export const QueryChatToolButtons: Component<QueryChatToolButtonsProps> = (props: QueryChatToolButtonsProps) => {
  ...
  const [openToolSource, setOpenToolSource] = createSignal<string | null>("infumap");
      ...
      <Show when={inspectorOpen()}>
        <Portal mount={document.body}>
          ...
          <div
            ...
            style={`left: 1vw; top: 1vh; width: 98vw; height: 98vh; z-index: ${Z_INDEX_GLOBAL_APP_OVERLAY + 1};`}
            role="dialog"
            ...
            <div class="min-h-0 grow overflow-y-auto px-3 py-2 text-[12px]">
              <details
                open={openToolSource() == "infumap"}
                onToggle={(ev) => {
                  if (ev.currentTarget.open) {
                    setOpenToolSource("infumap");
                  } else if (openToolSource() == "infumap") {
                    setOpenToolSource(null);
                  }
                }}>
              ...
              <For each={toolServers()}>{server => {
                const sourceId = `server:${server.id}`;
                return <details
                  open={openToolSource() == sourceId}
                  onToggle={(ev) => {
                    if (ev.currentTarget.open) {
                      setOpenToolSource(sourceId);
                    } else if (openToolSource() == sourceId) {
                      setOpenToolSource(null);
                    }
                  }}>
