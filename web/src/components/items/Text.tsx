...
export const Text: Component<VisualElementProps> = (props: VisualElementProps) => {
  ...
  const sizeBl = () => {
    if (props.visualElement.flags & VisualElementFlags.InsideCompositeOrDoc) {
      const cloned = TextFns.asTextMeasurable(ItemFns.cloneMeasurableFields(props.visualElement.displayItem));
      ...
      if (isPage(parentDisplayItem)) {
        ...
        cloned.spatialWidthGr = effectiveWidthGr != null
          ...
          : isLink(parentTreeItem)
            ...
            : panic(`Text sizeBl: parentTreeItem has unexpected type: ${parentTreeItem.itemType}`);
      }
    ...
    return TextFns.calcSpatialDimensionsBl(textItem());
  };
  ...
  const emoji = () => TextFns.emoji(textItem());
  ...
  const keyDownHandler = (ev: KeyboardEvent) => {
    switch (ev.key) {
      ...
      case "Escape":
        ev.preventDefault();
        ev.stopPropagation();
        commitActiveTextEdit(store, true, "text-escape-exit-edit");
        return;
    }
  }
  ...
  const enterKeyHandler = () => {
    ...
    if (ve.flags & VisualElementFlags.InsideTable || props.visualElement.actualLinkItemMaybe != null) {
      ...
      asTextItem(ve.displayItem).title = beforeText;
      ...
      arrangeNow(store, "text-enter-create-composite");
  ...
  // Check if this text is currently focused (via focusPath or textEditInfo)
  const isFocused = () => {
